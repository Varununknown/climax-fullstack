...
const QuizSystem: React.FC<QuizSystemProps> = ({ contentId, contentTitle }) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {/* Elegant Form Header */}
      <div className="text-center pb-6 sm:pb-8 border-b border-white/10">
        <div className="inline-flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-purple-600 to-fuchsia-600 flex items-center justify-center shadow-lg shadow-purple-500/50">
            <span className="text-xl sm:text-2xl">🎬</span>
          </div>
          <h3 className="text-2xl sm:text-3xl lg:text-4xl font-black bg-gradient-to-r from-purple-400 via-fuchsia-400 to-pink-400 bg-clip-text text-transparent">
            Fan Fest Challenge
          </h3>
        </div>
      ...
      <div className="space-y-4 sm:space-y-6">
        {questions.map((question, index) => (
          ...
          >
            {/* Question Card Glow */}
            <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-600 to-fuchsia-600 rounded-xl sm:rounded-2xl blur opacity-0 group-hover/question:opacity-20 transition-all duration-500"></div>
            
            {/* Question Card */}
            <div className="relative bg-gradient-to-br from-white/[0.07] to-white/[0.02] border border-white/10 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 backdrop-blur-sm hover:border-purple-500/30 transition-all duration-300">
              <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
                {/* Question Number Badge */}
                <div className="flex-shrink-0">
                  <div className="relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-purple-600 to-fuchsia-600 rounded-xl sm:rounded-2xl blur-md opacity-60"></div>
                    <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-purple-600 to-fuchsia-600 flex items-center justify-center shadow-lg">
                      <span className="text-xl sm:text-2xl font-black text-white">{index + 1}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-2 sm:space-y-3 pt-2">
                    {question.options.map((option, optionIndex) => {
                      ...
                      return (
                        ...
                        >
                          {/* Option Card */}
                          <div className={`flex-1 flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg sm:rounded-xl border-2 transition-all duration-300 ${
                            isSelected
      ...
      <div className="relative group/phone">
        <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-xl sm:rounded-2xl blur opacity-20 group-hover/phone:opacity-30 transition-all duration-500"></div>
        <div className="relative bg-gradient-to-br from-blue-900/30 to-cyan-900/30 border border-blue-500/40 backdrop-blur rounded-xl sm:rounded-2xl p-5 sm:p-6 lg:p-8">
          <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-5">
            ...
            <div className="flex-1 space-y-3 sm:space-y-4 w-full">
              ...
              <input
                ...
                inputMode="numeric"
                className="w-full px-4 sm:px-5 py-3 sm:py-4 bg-black/40 border-2 border-blue-400/30 rounded-lg sm:rounded-xl focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 text-white placeholder-gray-500 transition-all duration-300 text-sm sm:text-base font-medium"
              />
            </div>
          </div>
        </div>
      ...
      <div className="pt-4 sm:pt-6">
        ...
        >
          {/* Button Glow */}
          <div className={`absolute -inset-1 rounded-xl sm:rounded-2xl blur-xl transition-all duration-500 ${
          ...
          {/* Button */}
          <div className={`relative py-4 sm:py-5 px-6 sm:px-8 rounded-xl sm:rounded-2xl font-black text-base sm:text-lg lg:text-xl transition-all duration-300 ${
            Object.keys(answers).length === 0 || loading
